...
// GlobalSharkAttack SDK retry feature
...
class GlobalSharkAttackRetryFeature extends GlobalSharkAttackBaseFeature
{
    ...
    public function init(GlobalSharkAttackContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (GlobalSharkAttackContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(GlobalSharkAttackContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(GlobalSharkAttackContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
