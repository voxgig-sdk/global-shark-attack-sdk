...
// GlobalSharkAttack SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class GlobalSharkAttackTestFeature extends GlobalSharkAttackBaseFeature
{
    ...
    public function init(GlobalSharkAttackContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (GlobalSharkAttackContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = GlobalSharkAttackParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (GlobalSharkAttackContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(GlobalSharkAttackContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = GlobalSharkAttackParam::call($ctx, $k);
