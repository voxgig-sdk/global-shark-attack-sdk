...
// GlobalSharkAttack SDK feature test
//
...
class FtCtrl extends GlobalSharkAttackControl
{
...
class FtClient
{
    ...
    private GlobalSharkAttackUtility $utility;

    public function __construct(GlobalSharkAttackUtility $utility, array $options)
    {
    ...
    public function get_utility(): GlobalSharkAttackUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GlobalSharkAttackContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GlobalSharkAttackContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GlobalSharkAttackContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GlobalSharkAttackConfig::make_feature($name);
    ...
    private static function buildUrl(GlobalSharkAttackSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GlobalSharkAttackError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GlobalSharkAttackResponse($fetched);
        }
    ...
    private function fail(GlobalSharkAttackContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GlobalSharkAttackContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
