...
// GlobalSharkAttack SDK pipeline test
//
...
{
    private static function utility(): GlobalSharkAttackUtility
    {
        return new GlobalSharkAttackUtility();
    }
    ...
    private static function ctx(array $over = []): GlobalSharkAttackContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new GlobalSharkAttackContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new GlobalSharkAttackOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof GlobalSharkAttackError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new GlobalSharkAttackOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = GlobalSharkAttackMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new GlobalSharkAttackOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = GlobalSharkAttackMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new GlobalSharkAttackOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = GlobalSharkAttackMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GlobalSharkAttackSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = GlobalSharkAttackMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new GlobalSharkAttackResponse([]),
            'result' => new GlobalSharkAttackResult([]),
        ]);
        [, $err] = GlobalSharkAttackMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new GlobalSharkAttackSpec([]),
            'response' => null,
            'result' => new GlobalSharkAttackResult([]),
        ]);
        [, $err] = GlobalSharkAttackMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new GlobalSharkAttackSpec(['step' => 's']),
            'response' => new GlobalSharkAttackResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new GlobalSharkAttackResult([]),
        ]);
        [, $err] = GlobalSharkAttackMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new GlobalSharkAttackSpec(['step' => 's']),
            'response' => new GlobalSharkAttackResponse(self::resp(200, ['v' => 1])),
            'result' => new GlobalSharkAttackResult([]),
        ]);
        [, $err] = GlobalSharkAttackMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new GlobalSharkAttackSpec(['step' => 's']),
            'response' => new GlobalSharkAttackResponse(self::resp(200, ['v' => 2])),
            'result' => new GlobalSharkAttackResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        GlobalSharkAttackMakeResponse::call($ctx);
    ...
    {
        $preset = new GlobalSharkAttackResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new GlobalSharkAttackSpec([]),
            'response' => new GlobalSharkAttackResponse([]),
            'result' => new GlobalSharkAttackResult([]),
        ...
        [$got, $err] = GlobalSharkAttackMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new GlobalSharkAttackResult([])]);
        [, $err] = GlobalSharkAttackMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new GlobalSharkAttackSpec([]), 'result' => null]);
        [, $err] = GlobalSharkAttackMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GlobalSharkAttackSpec(['step' => 's']),
            'result' => new GlobalSharkAttackResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new GlobalSharkAttackOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GlobalSharkAttackMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GlobalSharkAttackSpec(['step' => 's']),
            'result' => new GlobalSharkAttackResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new GlobalSharkAttackOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GlobalSharkAttackMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GlobalSharkAttackResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new GlobalSharkAttackSpec([]),
            'result' => new GlobalSharkAttackResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = GlobalSharkAttackMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GlobalSharkAttackMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new GlobalSharkAttackError('boom', 'boom');
        $utility->fetcher = function (GlobalSharkAttackContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GlobalSharkAttackSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = GlobalSharkAttackMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (GlobalSharkAttackContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = GlobalSharkAttackMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(GlobalSharkAttackResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        GlobalSharkAttackMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (GlobalSharkAttackContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GlobalSharkAttackSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = GlobalSharkAttackMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new GlobalSharkAttackResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new GlobalSharkAttackSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = GlobalSharkAttackMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = GlobalSharkAttackMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = GlobalSharkAttackMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new GlobalSharkAttackResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, GlobalSharkAttackDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new GlobalSharkAttackResult(['ok' => false])]);
        $this->expectException(GlobalSharkAttackError::class);
        GlobalSharkAttackDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new GlobalSharkAttackResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', GlobalSharkAttackMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        GlobalSharkAttackMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new GlobalSharkAttackBaseFeature();
        $b = new GlobalSharkAttackBaseFeature();
        GlobalSharkAttackFeatureAdd::call($ctx, $a);
        GlobalSharkAttackFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): GlobalSharkAttackBaseFeature
    {
        $f = new GlobalSharkAttackBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        GlobalSharkAttackFeatureAdd::call($ctx, self::named_feature('a'));
        GlobalSharkAttackFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        GlobalSharkAttackFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        GlobalSharkAttackFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        GlobalSharkAttackFeatureAdd::call($ctx, $replace);
        ...
        GlobalSharkAttackFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): GlobalSharkAttackContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new GlobalSharkAttackSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GlobalSharkAttackPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        GlobalSharkAttackPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new GlobalSharkAttackResponse(['headers' => null]),
            'result' => new GlobalSharkAttackResult([]),
        ]);
        GlobalSharkAttackResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        GlobalSharkAttackResultBody::call($ctx);
