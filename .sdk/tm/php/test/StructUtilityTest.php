...
// GlobalSharkAttack SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = GlobalSharkAttackOmni::makeStructRunner(
    ...
    {
        return GlobalSharkAttackOmni::objmodel(GlobalSharkAttackOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return GlobalSharkAttackOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return GlobalSharkAttackOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(GlobalSharkAttackOmni::structfix($now, Struct::undef()))
                        !== json_encode(GlobalSharkAttackOmni::structfix($val, Struct::undef()))) {
