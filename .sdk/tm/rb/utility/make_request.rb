...
module GlobalSharkAttackUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = GlobalSharkAttackResponse.new({})
    ...
    if fetch_err
      ...
      response = GlobalSharkAttackResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GlobalSharkAttackResponse.new(fetched)
    else
