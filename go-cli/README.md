...
boru-driven command-line client **and** interactive REPL for the GlobalSharkAttack
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/global-shark-attack-cli)
make build

# 2. See usage (words, entities, env vars)
./global-shark-attack-cli --help
...
./global-shark-attack-cli list download

# 5. Override the API base URL for a single call
GLOBAL_SHARK_ATTACK_BASE=https://api.example.com ./global-shark-attack-cli list analyze

# 6. No arguments -> interactive REPL
./global-shark-attack-cli
global-shark-attack> list analyze
global-shark-attack> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/global-shark-attack-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/global-shark-attack-cli list analyze
   ```
...
Run with no arguments to open a REPL (prompt `global-shark-attack>`). Each line is
evaluated as its own boru expression:

```text
$ ./global-shark-attack-cli
global-shark-attack> list analyze
global-shark-attack> /help
global-shark-attack> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `analyze`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/global-shark-attack-cli`. |
...
(`./global-shark-attack-cli <expr>`) and interactively (the REPL), and expressions compose the
