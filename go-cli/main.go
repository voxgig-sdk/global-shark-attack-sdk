// Command GlobalSharkAttack-cli is a CLI + REPL for the GlobalSharkAttack SDK,
...
const prompt = "global-shark-attack"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "analyze download search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      GLOBAL_SHARK_ATTACK_APIKEY (api key), GLOBAL_SHARK_ATTACK_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: GLOBAL_SHARK_ATTACK_APIKEY carries the API key and
	// GLOBAL_SHARK_ATTACK_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GLOBAL_SHARK_ATTACK_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GLOBAL_SHARK_ATTACK_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGlobalSharkAttackSDK(opts)
