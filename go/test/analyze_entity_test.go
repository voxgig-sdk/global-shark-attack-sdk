...
func TestAnalyzeEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Analyze(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Analyze(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GLOBAL_SHARK_ATTACK_TEST_ANALYZE_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		analyzeRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.analyze")))
...
func analyzeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GLOBAL_SHARK_ATTACK_TEST_ANALYZE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GLOBAL_SHARK_ATTACK_TEST_ANALYZE_ENTID"])
