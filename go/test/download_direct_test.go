...
func TestDownloadDirect(t *testing.T) {
	t.Run("direct-list-download", func(t *testing.T) {
		setup := downloadDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-download", _mode); _shouldSkip {
			if _reason == "" {
...
func downloadDirectSetup(mockres any) *downloadDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"GLOBAL_SHARK_ATTACK_TEST_DOWNLOAD_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GLOBAL_SHARK_ATTACK_TEST_DOWNLOAD_ENTID"]; ok {
