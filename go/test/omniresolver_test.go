...
import (
	...
	omni "github.com/voxgig-sdk/global-shark-attack-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GlobalSharkAttackError); is {
				out := map[string]any{
					"name":    "GlobalSharkAttackError",
...
func omniCtx(arg any, client *sdk.GlobalSharkAttackSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GlobalSharkAttackSDK); is {
			client = live
