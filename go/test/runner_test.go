...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("GLOBALSHARKATTACK_TEST_LIVE") == "TRUE" ||
		os.Getenv("GLOBALSHARKATTACK_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("GLOBALSHARKATTACK_TEST_EXPLAIN"); explain != "" {
