...
The Lua SDK for the GlobalSharkAttack API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Analyze(nil):list(nil, nil)
...
local result, err = client:GlobalSharkAttack(nil):load(
  { id = "test01" }, nil
)
