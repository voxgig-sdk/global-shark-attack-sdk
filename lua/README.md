# GlobalSharkAttack Lua SDK



The Lua SDK for the GlobalSharkAttack API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Analyze()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/global-shark-attack-sdk/releases)),
...
for _, item in ipairs(analyzes) do
  print(item["x"])
end
```
...
Create an instance: `local analyze = client:Analyze(nil)`
...
```
lua/
├── global-shark-attack_sdk.lua    -- Main SDK module
...
The main module (`global-shark-attack_sdk`) exports the SDK constructor
