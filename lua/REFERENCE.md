# GlobalSharkAttack Lua SDK Reference

Complete API reference for the GlobalSharkAttack Lua SDK.
...
```lua
local results, err = client:Analyze(nil):list(nil, nil)
```
...
```lua
local results, err = client:Download(nil):list(nil, nil)
```
