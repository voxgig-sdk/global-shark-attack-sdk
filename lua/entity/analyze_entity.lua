...
local AnalyzeEntity = {}
AnalyzeEntity.__index = AnalyzeEntity


function AnalyzeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AnalyzeEntity)
  self._name = "analyze"
  self._client = client
...
function AnalyzeEntity:get_name()
  return self._name
end


function AnalyzeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AnalyzeEntity.new(self._client, opts)
end
...
function AnalyzeEntity:mark_deleted()
  self._deleted = true
end


function AnalyzeEntity:deleted()
  return true == self._deleted
end


function AnalyzeEntity:data_set(args)
  if args ~= nil then
...
function AnalyzeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AnalyzeEntity:match_set(args)
  if args ~= nil then
...
function AnalyzeEntity:match_get()
...
function AnalyzeEntity:stream(action, args, callopts)
...
function AnalyzeEntity:_run_op(ctx, post_done)
  local utility = self._utility
