...
require("global-shark-attack_types")
...
local GlobalSharkAttackSDK = {}
GlobalSharkAttackSDK.__index = GlobalSharkAttackSDK
...
GlobalSharkAttackSDK._make_feature = _make_feature


function GlobalSharkAttackSDK.new(options)
  local self = setmetatable({}, GlobalSharkAttackSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function GlobalSharkAttackSDK:options_map()
...
function GlobalSharkAttackSDK:get_utility()
  return Utility.copy(self._utility)
end


function GlobalSharkAttackSDK:get_root_ctx()
  return self._rootctx
end


function GlobalSharkAttackSDK:prepare(fetchargs)
...
function GlobalSharkAttackSDK:direct(fetchargs)
...
function GlobalSharkAttackSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function GlobalSharkAttackSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "GlobalSharkAttackSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function GlobalSharkAttackSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function GlobalSharkAttackSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "GlobalSharkAttackSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function GlobalSharkAttackSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = GlobalSharkAttackSDK.new(sdkopts)
  sdk.mode = "test"
