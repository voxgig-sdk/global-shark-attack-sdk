...
class GlobalSharkAttackSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = GlobalSharkAttackHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = GlobalSharkAttackHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!GlobalSharkAttackFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, GlobalSharkAttackFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return GlobalSharkAttackUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = GlobalSharkAttackHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = GlobalSharkAttackHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = GlobalSharkAttackHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new GlobalSharkAttackError($op . "_allow",
                "GlobalSharkAttackSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = GlobalSharkAttackHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new GlobalSharkAttackError("graphql_error",
                "GlobalSharkAttackSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_analyze = null;

    // Canonical facade: $client->Analyze()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->analyze()
    // resolves here too.
    public function Analyze($data = null)
    {
        require_once __DIR__ . '/entity/analyze_entity.php';
        if ($data === null) {
            if ($this->_analyze === null) {
                $this->_analyze = new AnalyzeEntity($this, null);
            }
            return $this->_analyze;
        }
        return new AnalyzeEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new GlobalSharkAttackSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
