...
class DownloadEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = GlobalSharkAttackSDK::test($seed, null);
        $seen = iterator_to_array($base->Download(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = GlobalSharkAttackConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = GlobalSharkAttackSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Download(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GLOBAL_SHARK_ATTACK_TEST_DOWNLOAD_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $download_ref01_list_result = $download_ref01_ent->list($download_ref01_match, null);
...
function download_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("GLOBAL_SHARK_ATTACK_TEST_DOWNLOAD_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["GLOBAL_SHARK_ATTACK_TEST_DOWNLOAD_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["GLOBAL_SHARK_ATTACK_TEST_LIVE"] === "TRUE") {
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["GLOBAL_SHARK_ATTACK_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
