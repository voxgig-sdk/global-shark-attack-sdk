...
//    only - a typed GlobalSharkAttackContext there would make every ctx
...
final class GlobalSharkAttackOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new GlobalSharkAttackContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new GlobalSharkAttackSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new GlobalSharkAttackResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new GlobalSharkAttackError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new GlobalSharkAttackResponse($clean['response']);
    ...
    public static function err_from_map($m): ?GlobalSharkAttackError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new GlobalSharkAttackError(is_string($code) ? $code : '', $msg);
    }
