...
class GlobalSharkAttackMakeError
{
    public static function call(?GlobalSharkAttackContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new GlobalSharkAttackContext([], null);
        }
        $op = $ctx->op ?? new GlobalSharkAttackOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new GlobalSharkAttackResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof GlobalSharkAttackError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new GlobalSharkAttackError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof GlobalSharkAttackError) {
