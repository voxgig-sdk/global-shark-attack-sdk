...
The Python SDK for the GlobalSharkAttack API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Analyze(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.GlobalSharkAttack(None).load(
    {"id": "test01"}, None
)
