# GlobalSharkAttack Python SDK Reference

Complete API reference for the GlobalSharkAttack Python SDK.
...
#### `GlobalSharkAttackSDK.test(testopts=None, sdkopts=None)`
...
Create a new `AnalyzeEntity` instance. Pass `None` for no initial data.
...
```python
results = client.Analyze().list({"dataset": "example"})
...
Create a new `AnalyzeEntity` instance with the same options.
...
```python
results = client.Download().list({"dataset": "example"})
...
```python
results = client.Search().list({"dataset": "example"})
