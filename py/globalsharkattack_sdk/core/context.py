...
from globalsharkattack_sdk.core.helpers import get_ctx_prop, to_map
...
class GlobalSharkAttackContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = GlobalSharkAttackControl()
        ...
        if isinstance(sp, GlobalSharkAttackSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, GlobalSharkAttackResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, GlobalSharkAttackResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return GlobalSharkAttackOperation({})
    ...
    def make_error(self, code, msg):
        return GlobalSharkAttackError(code, msg, self)
