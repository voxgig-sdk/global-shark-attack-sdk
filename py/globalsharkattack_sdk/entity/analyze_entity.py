...
from globalsharkattack_sdk.globalsharkattack_types import (
    Analyze,
    AnalyzeListMatch,
)
...
class AnalyzeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AnalyzeEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Analyze]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Analyze().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
