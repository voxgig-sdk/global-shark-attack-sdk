# GlobalSharkAttack SDK test runner

from __future__ import annotations
import os
import json

from globalsharkattack_sdk.utility.voxgig_struct import voxgig_struct as vs


class GlobalSharkAttackTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GlobalSharkAttackTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GlobalSharkAttackTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GlobalSharkAttackTestRunner.getenv("GLOBAL_SHARK_ATTACK_TEST_LIVE")
        override = GlobalSharkAttackTestRunner.getenv("GLOBAL_SHARK_ATTACK_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GlobalSharkAttackTestRunner.getenv(key)
        ...
        explain = GlobalSharkAttackTestRunner.getenv("GLOBAL_SHARK_ATTACK_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if GlobalSharkAttackTestRunner._test_control is not None:
            return GlobalSharkAttackTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GlobalSharkAttackTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GlobalSharkAttackTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GlobalSharkAttackTestRunner.load_test_control()
...
def load_env_local():
    GlobalSharkAttackTestRunner.load_env_local()


def env_override(m):
    return GlobalSharkAttackTestRunner.env_override(m)


def entity_data(v):
    return GlobalSharkAttackTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GlobalSharkAttackTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GlobalSharkAttackTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GlobalSharkAttackTestRunner.load_test_control()


def live_client_options():
    return GlobalSharkAttackTestRunner.live_client_options()


def live_delay_ms():
    return GlobalSharkAttackTestRunner.live_delay_ms()
