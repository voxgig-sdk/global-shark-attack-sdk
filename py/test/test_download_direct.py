...
class TestDownloadDirect:
    ...
    def test_should_direct_list_download(self):
        setup = _download_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-download", "live" if setup["live"] else "unit")
        if _skip:
