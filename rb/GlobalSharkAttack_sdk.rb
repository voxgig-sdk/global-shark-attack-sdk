...
require_relative 'GlobalSharkAttack_types'
...
class GlobalSharkAttackSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = GlobalSharkAttackHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = GlobalSharkAttackHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, GlobalSharkAttackFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    GlobalSharkAttackUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = GlobalSharkAttackHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = GlobalSharkAttackHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = GlobalSharkAttackHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => GlobalSharkAttackError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue GlobalSharkAttackError => err
    ...
    if fetched.is_a?(Hash)
      status = GlobalSharkAttackHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = GlobalSharkAttackError.new(
        "graphql_error", "GlobalSharkAttackSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = GlobalSharkAttackSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
