# GlobalSharkAttack Ruby SDK



The Ruby SDK for the GlobalSharkAttack API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Analyze` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
require_relative "GlobalSharkAttack_sdk"
client = GlobalSharkAttackSDK.new(options)
```
...
```ruby
client = GlobalSharkAttackSDK.test(testopts, sdkopts)
```
...
raise a `GlobalSharkAttackError` (a `StandardError` subclass), so wrap
...
```
rb/
├── GlobalSharkAttack_sdk.rb       -- Main SDK module
...
The main module (`GlobalSharkAttack_sdk`) exports the SDK class
