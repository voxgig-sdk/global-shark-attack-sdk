...
class AnalyzeEntity
  ...
  def make
    opts = @_entopts.dup
    AnalyzeEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = GlobalSharkAttackHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Analyze, Hash] the current Analyze data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = GlobalSharkAttackHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Analyze fields)
  def match_get
  ...
  #
  # @param reqmatch [AnalyzeListMatch, Hash, nil] match filter (any subset of
  #   Analyze fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Analyze>, Array] the matching Analyze items; raises GlobalSharkAttackError on failure
