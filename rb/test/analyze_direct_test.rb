...
class AnalyzeDirectTest < Minitest::Test
  def test_direct_list_analyze
    setup = analyze_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-analyze", setup[:live] ? "live" : "unit")
    if _should_skip
...
def analyze_direct_setup(mockres)
  ...
  if live
    ...
    client = GlobalSharkAttackSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
