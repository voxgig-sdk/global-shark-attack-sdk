...
describe('AnalyzeDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new GlobalSharkAttackSDK({
  ...
  test('direct-list-analyze', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-analyze', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GLOBAL_SHARK_ATTACK_TEST_ANALYZE_ENTID']
