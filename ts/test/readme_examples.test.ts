...
//     (\`new GlobalSharkAttackSDK(...)\` / \`GlobalSharkAttackSDK.test(...)\` become
//     \`GlobalSharkAttackSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'GlobalSharkAttackSDK'
...
const TEST_SEED = {"entity":{"analyze":{"test01":{"id":"test01"}},"download":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new GlobalSharkAttackSDK(...)\` and any
// \`GlobalSharkAttackSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+GlobalSharkAttackSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/GlobalSharkAttackSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
