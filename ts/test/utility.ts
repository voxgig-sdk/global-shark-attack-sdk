...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.GLOBALSHARKATTACK_TEST_LIVE ||
    'TRUE' === process.env.GLOBALSHARKATTACK_TEST_OVERRIDE
  ) {
  ...
  m.GLOBALSHARKATTACK_TEST_EXPLAIN = process.env.GLOBALSHARKATTACK_TEST_EXPLAIN || m.GLOBALSHARKATTACK_TEST_EXPLAIN
